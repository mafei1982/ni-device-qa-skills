...
The PXIe-4142 User Manual provides detailed descriptions of the product functionalityand the step by step processes for use.

# PXIe-4142 Overview

The PXIe-4142 is a four-channel, four-quadrant source measure unit (SMU) designedfor engineers building PXI systems that require voltage or current sourcing andmeasurement. Use the PXIe-4142 in high pin-count applications.
...
<table><tr><td>Driver Name</td><td>Earliest Version Support</td></tr><tr><td>NI-DCPower</td><td>1.7</td></tr></table>

# Components of a PXIe-4142 System

The PXIe-4142 is designed for use in a system that includes other hardwarecomponents, drivers, and software.


Notice A system and the surrounding environment must meet therequirements defined in PXIe-4142 Specifications.

The following list defines the minimum required hardware and software for a systemthat includes a PXIe-4142.


Table 2. System Components


<table><tr><td>Component</td><td>Description and Recommendations</td></tr><tr><td rowspan="2">PXI Chassis</td><td>A PXI chassis houses the PXIe-4142 and supplies power, communication, and timing for PXIe-4142 functions.</td></tr><tr><td>Note When installing the PXIe-4142 in a chassis with slot cooling capacity = 38 W, set the chassis fan speed to HIGH.</td></tr><tr><td>PXI Controller or PXI Remote Control Module</td><td>You can install a PXI controller or a PXI remote control (MXI) module depending on your system requirements. These components, installed in the same PXI chassis as the PXIe-4142, interface with the SMU using NI device drivers.</td></tr><tr><td>SMU</td><td>Your SMU instrument.</td></tr><tr><td>Cables and Accessories</td><td>Cables and accessories allow connectivity to/from your instrument for measurements. Refer to Cables and Accessories for recommended cables and accessories and guidance.</td></tr><tr><td>NI-DCPower Driver</td><td>Instrument driver software that provides functions to interact with the PXIe-4142 and execute measurements using the PXIe-4142.
Note NI recommends to always use the most current version of NI-DCPower with the PXIe-4142. You can find the NI-DCPower driver requirements in the NI-DCPower Readme.</td></tr><tr><td>NI Applications</td><td>NI-DCPower offers driver support for the following applications:
• InstrumentStudio
• LabVIEW
...
A measurement circuit on the PXIe-4142 can simultaneously read the voltage andcurrent values using two integrating analog-to-digital converters. Voltage is measureddifferentially between the HI and LO terminals (local sense) or between the Sense HIand Sense LO terminals (remote sense) based on the programmed voltage senselocation. Remote sense is used to compensate for voltage drop that results fromresistance in cables, connectors and switches. Current is measured using shuntresistors in series with the HI terminal.

Additionally, the PXIe-4142 features a Guard terminal on the output connector. You canuse the Guard terminal to implement guarding techniques against parasitic leakageresistance and capacitance in cabling and test fixtures.

There are several protection mechanisms built into the PXIe-4142 that guard againstcommon faults. The output has an over-current protection (OCP) circuit that will openthe Output Disconnect switch when an over-current condition is either too severe orlasts too long. The Output Disconnect switch on PXIe-4142 is not directly controllableby the user and reserved for protection and self-calibration use.
...
The output terminals of the PXIe-4142 are electrically isolated from chassis groundthrough a 60 V DC, Category I isolation barrier. This allows any SMU terminal to float±60 V DC with respect to chassis ground.

The PXIe-4142 includes flexible source and measurement units that enable multipleprogramming modes and timing options:
...
# PXIe-4142 Block Diagram

The following diagram illustrates the design of the PXIe-4142.
...
# PXIe-4142 LED Indicators

The PXIe-4142 features an Access LED and Active LED.
...
The LEDs may not light until the module has been configured in HardwareConfiguration Utility or MAX. Before proceeding, verify that the PXIe-4142 appears inHardware Configuration Utility or MAX.
...
Notice Apply external signals only while the PXIe-4142 is powered on.Applying external signals while the module is powered off may causedamage.
...
# PXIe-4142 Installation and Configur tion and Configuration

Complete the following steps to install the PXIe-4142 into a chassis and prepare it foruse.
...
3. Installing the PXIe-4142 into a ChassisInstall the instrument into a PXI chassis.
...
6. Self-Calibrating the PXIe-4142 in MAXSelf-calibration adjusts the PXIe-4142 for variations in the module environment.The PXIe-4142 modules are externally calibrated at the factory, however youshould perform a complete self-calibration after you install the module.
...
# Installing the PXIe-4142 into a Chassis

Install the instrument into a PXI chassis.


Notice To prevent damage to the PXIe-4142 caused by ESD orcontamination, handle the module using the edges or the metal bracket.
...
6. Identify a supported slot in the chassis. The PXIe-4142 module can be placed inPXI Express hybrid peripheral slots $( \bullet ^ { \mathsf { H } } )$ , PXI Express system timing slots $( \boxed { \bullet } )$or PXI Express peripheral slots $( \textcircled { > } )$ .
...
The PXIe-4142 offers two output accessories that can be attached to the front panelconnector:

• Output Connector—This output connector ships with the PXIe-4142.

• TB-414x—This optional adapter is used to convert the 25-position D-SUBconnector of the PXIe-4142 to screw terminal connections while maintaining lowleakage performance.

# Installing the Output Connec alling the Output Connector Assembly ont ssembly onto the PXIe-4142

Complete the following steps to install the output connector onto the PXIe-4142.
...
Note Low energy transients can appear at the output terminals of yourPXIe-4142 during certain situations, such as power-up, power-down,device driver loading, and self-calibration. Additionally in thesesituations, the output is pulled to ground through a $1 0 \mathsf { k } \Omega$ resistor.
...
Note Maintaining low leakage performance of the PXIe-4142 and TB-414Xrequires careful selection and termination of the external wiring to ensureguarding of the HI and HI Sense signals and to minimize negative effects fromwiring insulation materials and contaminants. The SHDB25F-DB25F-LL cableaccessory has been designed with these considerations in mind and utilizesthe TB-414X, allowing for one end to be easily reterminated into an alternateconnector, if needed.
...
Refer to the following pinout, signal information, and PXIe-4142 pin map to makethe appropriate connections for your application.
...
PXIe-4142 during certain situations, such as power-up, power-down,device driver loading, and self-calibration. Additionally in thesesituations, the output is pulled to ground through a $1 0 \mathsf { k } \Omega$ resistor.
...
Note If you do not see your module listed, press $\tt { < F 5 > }$ to refresh the listof installed modules. If the module is still not listed, power off the system,

ensure the module is correctly installed, and restart.

4. Record the name MAX assigns to the hardware. Use this identifier whenprogramming the PXIe-4142.
...
# What Should I Do if the PXIe-4142 Does Not Appear in MAX?
...
5. Verify the PXIe-4142 appears in the Device Manager.

a. Under an NI entry, confirm that a PXIe-4142 entry appears.
...
# What Should I Do if the PXIe-4142 Fails the Self-Test?

1. Reset the PXIe-4142 through Hardware Configuration Utility or MAX and thenperform the self-test again.
...
# Self-Calibrating the PXIe-4142 in MAX

Self-calibration adjusts the PXIe-4142 for variations in the module environment. ThePXIe-4142 modules are externally calibrated at the factory, however you shouldperform a complete self-calibration after you install the module.

1. Install the PXIe-4142 and let it warm up for the recommended warm-up time listedin the PXIe-4142 Specifications.
...
3. Self-calibrate the PXIe-4142 by clicking the Self-Calibrate button in MAX or callingniDCPower Cal Self Calibrate (niDCPower_CalSelfCalibrate).


Note Low energy transients can appear at the output terminals of yourPXIe-4142 during certain situations, such as power-up, power-down,device driver loading, and self-calibration. Additionally in thesesituations, the output is pulled to ground through a $1 0 \mathsf { k } \Omega$ resistor.

The energy of the transients is typically less than $1 \mu \mathrm { J }$ , which is notenough to cause damage to most loads. However, some extremelysensitive devices could be affected. Additionally, the presence of the

10 kΩ pulldown to ground could pose a problem for some loads. If yoususpect your load could be affected, NI recommends that you disconnectall power supplies and SMU connections while performing any of theabove operations. To avoid transients caused by a power failure, consideremploying an uninterruptible power supply system.

# Connecting Signals to the PXIe-4142

Refer to the following topics for guidance about PXIe-4142 signal connections.
...
When the PXIe-4142 is operating in Constant Voltage mode, local sense forces therequested voltage at the output terminals of the module. The actual voltage at theDUT terminals is lower than the requested output because of the output leadresistance error.
...
Remote sense measurements, or 4-wire senses, require 4-wire connections to the DUT.Optionally, remote sense measurements require 4-wire switches if a switching systemis used to expand the channel count.

A remote sense configuration works in the following way:

• One set of leads carries the output current.

• Another set of leads measures voltage directly at the DUT terminals.
...
The current flowing in the output leads can be several amps or more. However,depending on the instrument, only low current flows through the sense leads. The lowcurrent results in smaller voltage drop error for measurements compater to the localsense error. When using remote sense in the DC Voltage output function, the outputvoltage is forced at the end of the sense leads instead of the output terminals. Whenusing remote sense in the DC Current output function, the voltage limit is measured atthe end of the sense leads instead of at the output terminals. Using remote senseresults in a voltage at the DUT terminals that is more accurate than what local sensecan achieve. Ideally, the sense leads are connected as close to the DUT terminals aspossible.

When using remote sense, the voltage drop across the higher current output leads istypically limited to one or two volts per lead. This limit depends on the specific powersupply or SMU (Source Measure Unit) you are using. If you apply a voltage higher thanthe specified maximum across the output leads while using the DC Voltage outputfunction in remote sense mode, the voltage at the load might be lower than expected.

Notice When attempting to force a current using the DC Current outputfunction in local or remote sense, excessive line drop might occur. The linedrop can force the power supply or SMU into Constant Voltage mode beforereaching the requested current level.

If you configure a channel for remote sense operation without connecting the senseleads to the DUT, the measurements might not meet the published specifications. Ifyou configure a channel for remote sense and the remote sense leads are left open, thechannel might source a voltage higher than the voltage level or voltage limit.

Refer to the PXIe-4142 Specifications for more information about remote sensesupport and the maximum output lead voltage drop allowed.

The PXIe-4142 features internal open-sense protection through a $1 0 0 \mathsf { k } \Omega$ resistorbetween the force (Output HI/LO) and sense (Sense HI/LO) lines. This protectionprovides a secondary measurement path to regulate the instrument output if remotesense becomes disconnected.
...
Caution Do not connect Guard to the outer shield of coaxial cable if theguard potential will exceed 30 V RMS, 42.4 V peak, or 60 V DC on the outer

shield. Use a triaxial cable where higher voltages are needed, and connectthe Guard to the inner shield. All the Guard terminals for a given channel areequivalent and are always enabled.
...
The PXIe-4142 channels can generate voltage and current in Single Point orSequence source mode.
...
The Source Mode With Channels function defines the source mode the PXIe-4142channels are operating in.
...
The PXIe-4142 can perform operations to source and measure voltage and current. Inorder to perform these operations, use the NI-DCPower driver to configure softwaresettings and execute operations.

Refer to the following table for an overview of common source and measureoperations as well as the software setting combinations that enable the PXIe-4142 toperform each operation.


Table 10. Software Settings for PXIe-4142 Source and Measure Operations


<table><tr><td rowspan="2">PXle-4142 Operation</td><td colspan="2">Software Settings</td></tr><tr><td>Output Function</td><td>Source Mode</td></tr><tr><td>Source voltage</td><td rowspan="2">DC Voltage</td><td rowspan="4">Single Point or Sequence</td></tr><tr><td>Measure current or voltage</td></tr><tr><td>Source current</td><td rowspan="2">DC Current</td></tr><tr><td>Measure voltage or current</td></tr></table>

Complete the following general steps to source current or voltage.

1. Initialize a Session

Use the NI-DCPower driver to initialize a session with the PXIe-4142.

2. Configure the PXIe-4142 for Sourcing

Use the NI-DCPower driver with the PXIe-4142 to control the output the instrumentgenerates. Depending on the output function and source mode, you can configurethe appropriate output levels and limits.

3. Configure the PXIe-4142 for Measuring

Once you configure channels and they are in the Running state, the PXIe-4142 cantake measurements.
...
5. Initiate the PXIe-4142 for Sourcing and Measuring

Initiate the channels of the PXIe-4142 to apply a configuration and start generating.

6. Acquire Measurements

The applied channel configuration determines how the PXIe-4142 acquiresmeasurements.

7. Cease Generation

NI-DCPower includes one option for stopping generation on PXIe-4142 channelsand returning the channels to a known state.

8. Close the Session

Use the NI-DCPower driver to close a session with the PXIe-4142.
...
For any application you write, you must open a session to establish communicationwith the PXIe-4142 or specified channel(s) by initializing.
...
# Configure the PXIe-4142 for Sourcing

Use the NI-DCPower driver with the PXIe-4142 to control the output the instrumentgenerates. Depending on the output function and source mode, you can configure theappropriate output levels and limits.
...
# Configure the PXIe-4142 for Measuring

Once you configure channels and they are in the Running state, the PXIe-4142 can takemeasurements.
...
Refer to the PXIe-4142 Specifications for the trigger delay and jitter of yourinstrument.
...
# Initiate the PXIe-4142 for Sourcing and Measuring
...
The PXIe-4142 automatically acquires measurements when you configure thefollowing VIs or functions:
...
NI-DCPower includes one option for stopping generation on PXIe-4142 channels andreturning the channels to a known state.
...
The output of a channel is enabled by default when the channel enters the Runningstate. However, you can programmatically enable and disable the output channel(s) ofthe PXIe-4142.

When you disable the output of the PXIe-4142, the instrument is configured to output aDC voltage at 0 V with current limits at $\pm 2 \%$ of the presently configured current limitrange in, unless otherwise noted, a low-impedance state.
...
Refer to the following sections for information about PXIe-4142 features and guidelinesfor operating the PXIe-4142.
...
Because of the required power dissipation, sourcing and sinking capabilities for achannel are not always identical. Refer to the PXIe-4142 Specifications for more
...
Cable inductance has an effect on the output current slew rate. You can decreasecabling inductance and increase the speed of the PXIe-4142.
...
You can program the channel of the PXIe-4142 to vary the output resistance.
...
Use the niDCPower Configure Output Resistance VI or theniDCPower_Configure_Output_Resistance function to set the outputresistance. Refer to the PXIe-4142 Specifications for more information about thevalues to which you can set the output resistance.

# Overload Protection (OLP) tion (OLP)

The PXIe-4142 is protected against overcurrent (OCP) conditions and overvoltage(OVP) conditions.
...
channel and disconnects the channel circuitry from the output connector pins. Byinternally disconnecting the output, it protects both the PXIe-4142 and the deviceunder test (DUT).
...
Do not apply voltages at the output that exceed the ratings of the PXIe-4142. Refer tothe PXIe-4142 Specifications for information about voltage ratings.
...
The following table lists the PXIe-4142 transient response settings available in NI-DCPower.
...
Caution Power supplies not designed for four-quadrant operation maybecome damaged if reverse currents are applied to their output terminals.Reverse currents can cause the device to move into an unregulated modeand can damage the instrument. Refer to the PXIe-4142 Specificationsfor more information about channel capabilities.
...
Use the highest resolution (smallest) range possible for a particular application to getmaximum output and measurement accuracy. Refer to the PXIe-4142
...
You can reject the frequency of noise by adjusting the aperture time to be a multiple ofan AC noise frequency with Period = 1/f.

# Normal DC Measurement Noise Rejection

With normal noise rejection, the instrument assigns equal weight to each sample. This

setting mimics the behavior of most traditional power supplies and SMUs.
...
Directly adjusting the aperture time of your measurements allows you to reject specific

AC noise frequencies in your DC measurements with NI-DCPower.
...
The PXIe-4142 supports configurable step duration for sequences via the SequenceStep Delta Time property.
...
• Source generation time increases if a voltage or current range change is required,

which can delay generation of the new setpoint. The channel changes ranges if thevoltage or current ranges you specify for the step differ from the previouslyconfigured ranges.
...
·Measure Record Length = 1</td><td>Sequence Step dt – Minimum Sequence Step dt &lt; Source Delay + Aperture Time + Measure Complete Event Delay</td></tr><tr><td rowspan="2">·Sequence Step dt Enabled is True; 
·Measure When is set to Automatically After Source Complete; and 
·Measure Record Length &gt; 1; and 
·DC Noise Rejection is set to Normal</td><td>Sequence Step dt – Minimum Sequence Step dt &lt; Source Delay + Aperture Time + ((Measure Record Length - 1) × Measure Record dt) + Measure Complete Event Delay</td></tr><tr><td>Measure Record dt = Aperture Time</td></tr><tr><td>·Sequence Step dt</td><td>Minimum Sequence Step dt – Minimum Sequence Step dt &lt; Source Delay + Aperture Time + ((Measure Record Length - 1) ×</td></tr><tr><td rowspan="2">Enabled is True; 
• Measure When is set to Automatically After Source Complete; and 
• Measure Record Length &gt; 1; and 
• DC Noise Rejection is set to Second-Order</td><td>Measure Record dt) + Measure Complete Event Delay</td></tr><tr><td>Measure Record dt = Aperture Time ÷ 2</td></tr></table>

# Power Measurements

Each channel of the PXIe-4142 has two synchronized ADCs that measure voltage andcurrent. You can use NI-DCPower to measure power flowing to or from the PXIe-4142.

You can use the following VIs and functions to measure both current and voltage forboth channels of the PXIe-4142.
...
Power can be computed as the product of the voltage and the current. If the powermeasurement is positive, the PXIe-4142 is sourcing power. If the power measurementis negative, the PXIe-4142 is sinking power.
...
To measure a resistance with an NI power supply or SMU, select a test current thatcreates a voltage drop within module capabilities. After the channel output is enabled

and settled, use the niDCPower Measure Multiple VI or the
...
Refer to the following terms when learning more about the features and usage of thePXIe-4142:
...
Refer to the PXIe-4142 Specifications for measurement resolution and outputresolution information.
...
Refer to the PXIe-4142 Specifications for the following information for yourinstrument:
...
Refer to the PXIe-4142 Calibration Procedure for the external calibrationprocedure for your instrument.
...
• After first installing the PXIe-4142 in a chassis

• After any module that is in the same chassis as the PXIe-4142 is installed,uninstalled, or moved

• When the PXIe-4142 is in an environment where the ambient temperature changes.Refer to the PXIe-4142 Specifications to find the allowable ambienttemperature for your instrument.

• When the PXIe-4142 temperature has drifted outside of the specified $\mathsf { T } _ { \mathsf { C a l } }$ since thelast self-calibration. Refer to the PXIe-4142 Specifications to find the allowabledifference from $\mathsf { T } _ { \mathsf { C a l } }$ for this instrument.
