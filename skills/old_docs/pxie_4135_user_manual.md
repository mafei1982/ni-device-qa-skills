...
The PXIe-4135 is a high-precision system SMU that has the following features andcapabilities.
...
<table><tr><td>Variant</td><td>Driver Name</td><td>Earliest Version Support</td></tr><tr><td>PXIe-4135 (20 W)</td><td>NI-DCPower</td><td>15.2</td></tr><tr><td>PXIe-4135 (40 W)</td><td>NI-DCPower</td><td>20.7</td></tr></table>
...
The following list defines the minimum required hardware and software for a systemthat includes a PXIe-4135.


Table 2. System Components


<table><tr><td>Component</td><td>Description and Recommendations</td></tr><tr><td rowspan="3">PXI Chassis</td><td>A PXI chassis houses the PXIe-4135 and supplies power, communication, and timing for PXIe-4135 functions.</td></tr><tr><td>Note NI recommends installing the PXIe-4135 (40 W) in a chassis with slot cooling capacity ≥58 W for increased module capability.</td></tr><tr><td>Note When installing the PXIe-4135 in a chassis with slot cooling capacity =38 W, set the chassis fan speed to HIGH.</td></tr><tr><td>PXI Controller or PXI Remote Control Module</td><td>You can install a PXI controller or a PXI remote control (MXI) module depending on your system requirements. These components, installed in the same PXI chassis as the PXIe-4135, interface with the SMU using NI device drivers.</td></tr><tr><td></td><td></td></tr><tr><td>SMU</td><td>Your SMU instrument.</td></tr><tr><td>Cables and Accessories</td><td>Cables and accessories allow connectivity to/from your instrument for measurements. Refer to Cables and Accessories for recommended cables and accessories and guidance.</td></tr><tr><td>NI-DCPower Driver</td><td>Instrument driver software that provides functions to interact with the PXIe-4135 and execute measurements using the PXIe-4135.
Note NI recommends to always use the most current version of NI-DCPower with the PXIe-4135. You can find the NI-DCPower driver requirements in the NI-DCPower Readme.</td></tr><tr><td>NI Applications</td><td>NI-DCPower offers driver support for the following applications:
· InstrumentStudio
· LabVIEW
· LabWindows/CVI
· C/C++
...
• Install PXI slot blockers (p/n 199198-01) to fill empty instrument slots in a PXIchassis. For more information about installing slot blockers and filler panels, go to
...
A measurement circuit on the PXIe-4135 can simultaneously read the voltage andcurrent values using two integrating analog-to-digital converters. Voltage is measureddifferentially between the HI and LO terminals (local sense) or between the Sense HIand Sense LO terminals (remote sense) based on the programmed voltage senselocation. Remote sense is used to compensate for voltage drop that results fromresistance in cables, connectors and switches. Current is measured using shuntresistors in series with the HI terminal.

Additionally, the PXIe-4135 features a Guard terminal on the output connector. You canuse the Guard terminal to implement guarding techniques against parasitic leakageresistance and capacitance in cabling and test fixtures.

There are several protection mechanisms built into the PXIe-4135 that guard againstcommon faults. The output has an over-current protection (OCP) circuit that will openthe Output Disconnect switch when an over-current condition is either too severe or

lasts too long.

The PXIe-4135 continuously monitors voltage between HI and LO or HI Sense and LOSense and protects against over-voltage faults. If an excessive voltage is detected, theover-voltage protection (OVP) circuit opens the Output Disconnect switch to protectthe PXIe-4135 from excessive over-voltage.
...
The output terminals of the PXIe-4135 are electrically isolated from chassis groundthrough a 250 V DC, Category I isolation barrier. This allows any SMU terminal to float±250 V DC with respect to chassis ground.

The PXIe-4135 includes flexible source and measurement units that enable multipleprogramming modes and timing options:
...
The following figures show the terminals on the PXIe-4135 output connector andtriaxial connectors.
...
<table><tr><td>Signal</td><td>Description</td></tr><tr><td>Output HI</td><td>HI force terminal connected to channel power stage (generates and/or dissipates power). Positive polarity is defined as voltage measured on HI &gt; LO.</td></tr><tr><td>Inner Shield, Guard</td><td>Buffered output that follows the voltage of the HI force terminal. Used to drive shield conductors surrounding HI force and Sense HI conductors to minimize effects of leakage and capacitance on low level currents.</td></tr><tr><td>Outer Shield, Chassis Ground</td><td>Tied to chassis ground through module front panel. Connects to outer shield of triax cables and can form part of the LO force path when LO is grounded.</td></tr><tr><td>Sense HI</td><td>Voltage remote sense input terminals. Used to compensate for I*R voltage drops in cable leads, connectors and switches.</td></tr></table>
...
The LEDs may not light until the module has been configured in MAX. Beforeproceeding, verify that the PXIe-4135 appears in MAX.
...
<table><tr><td>Status Indicator</td><td>Output Channel State</td><td>Safety Interlock State</td></tr><tr><td>(Off)</td><td>The device output is disconnected from the voltage generation source through output disconnect relays.</td><td>Either open or closed.</td></tr><tr><td>Green</td><td>The device output is connected to the voltage generation source and &lt;42.4 V DC is present.</td><td>Open; only &lt;42.4 V DC is present.</td></tr><tr><td>Amber</td><td>The device output is connected to the voltage generation source and ≥42.4 V DC is present. High voltage may be generated by the device itself or an external device.</td><td>Closed; instrument can output up to its maximum voltage.</td></tr><tr><td>Red</td><td>The device has a fault or is in error due to the voltage generated or measured by the device. Refer to the driver</td><td>Open, and instrument is programmed to output ≥42.4 V DC.</td></tr><tr><td></td><td>software for possible sources.
The device will not operate until the error is cleared and/or the device is reset. High voltage may be present on the device.</td><td></td></tr></table>

# PXIe-4135 Installation and Configuration

Complete the following steps to install the PXIe-4135 into a chassis and prepare it foruse.

1. Unpacking the Kit

2. Installing the Software

3. Installing the PXIe-4135 into a Chassis

4. Installing the Output Connector Assembly onto the PXIe-4135

5. Verifying the Installation in MAX

6. Self-Calibrating the PXIe-4135 in MAXSelf-calibration adjusts the PXIe-4135 for variations in the module environment.The PXIe-4135 modules are externally calibrated at the factory, however youshould perform a complete self-calibration after you install the module.
...
2. PXIe-4135 Output Connector Assembly
...
6. Identify a supported slot in the chassis. The PXIe-4135 module can be placed inPXI Express hybrid peripheral slots $( \bullet ^ { \mathsf { H } } )$ , PXI Express system timing slots $\textcircled { < } \textcircled { < } \textcircled { > }$or PXI Express peripheral slots $( 0 )$ .
...
13. Connect the triaxial cables to the module.

# Installing the Output Connector Assembly onto thePXIe-4135
...
2. Insert each cable into the appropriate connector terminal. Terminate the cabledrain wire to the Chassis Ground terminal.

3. Inspect the terminal connections to ensure that no exposed metal is visible on theOutput LO and Sense LO cables beyond the body of the connector.

4. Clamp down using the screw terminals on the connector, and verify that no

exposed shield foil or braid extends outside of the backshell.

5. Using the cable tie, provide strain relief for the cable to the backshell.

6. Close the output connector assembly and tighten the retention screw to hold it inplace.

7. Connect the output connector assembly to the device with triaxial cables.

8. Tighten any thumbscrews on the output connector assembly to hold it in place.
...
4. Record the identifier MAX assigns to the hardware. Use this identifier whenprogramming the PXIe-4135.
...
# What Should I Do if the PXIe-4135 Does Not Appear in MAX?
...
5. Verify the PXIe-4135 appears in the Device Manager.

a. Under an NI entry, confirm that a PXIe-4135 entry appears.
...
# What Should I Do if the PXIe-4135 Fails the Self-Test?

1. Reset the PXIe-4135 through MAX, and then perform the self-test again.
...
# Self-Calibrating the PXIe-4135 in MAX

Self-calibration adjusts the PXIe-4135 for variations in the module environment. ThePXIe-4135 modules are externally calibrated at the factory, however you shouldperform a complete self-calibration after you install the module.
...
Refer to the following topics for guidance about PXIe-4135 signal connections.
...
Refer to the PXIe-4135 Specifications for more information about remote sensesupport and the maximum output lead voltage drop allowed.

The PXIe-4135 features internal open-sense protection through a 1 MΩ resistorbetween the force (Output HI/LO) and sense (Sense HI/LO) lines. This protectionprovides a secondary measurement path to maintain the instrument output inregulation if remote sense becomes disconnected.
...
The PXIe-4135 channels can generate voltage and current in Single Point orSequence source mode.
...
The Source Mode With Channels function defines the source mode the PXIe-4135channels are operating in.
...
The PXIe-4135 can output configurable current pulses and/or voltage pulses.
...
The PXIe-4135 supports in-range pulsing and extended range pulsing.

• In-range—Pulses fall within DC range limits.

• Extended range—Pulses fall outside DC range limits for either current or power.Extended range pulsing is subject to additional limitations as described in thePXle-4135 Specifications.

# Sourcing Voltage and Current

The PXIe-4135 can perform operations to source and measure voltage and current. Inorder to perform these operations, use the NI-DCPower driver to configure softwaresettings and execute operations.

Refer to the following table for an overview of common source and measureoperations as well as the software setting combinations that enable the PXIe-4135 toperform each operation.


Table 9. Software Settings for PXIe-4135 Source and Measure Operations


<table><tr><td rowspan="2">PXle-4135 Operation</td><td colspan="2">Software Settings</td></tr><tr><td>Output Function</td><td>Source Mode</td></tr><tr><td>Source voltage</td><td rowspan="2">DC Voltage</td><td rowspan="4">Single Point or Sequence</td></tr><tr><td>Measure current or voltage</td></tr><tr><td>Source current</td><td rowspan="2">DC Current</td></tr><tr><td>Measure voltage or current</td></tr></table>

Complete the following general steps to source current or voltage.

1. Initialize a Session

Use the NI-DCPower driver to initialize a session with the PXIe-4135.

2. Configure the PXIe-4135 for Sourcing

Use the NI-DCPower driver with the PXIe-4135 to control the output the instrumentgenerates. Depending on the output function and source mode, you can configurethe appropriate output levels and limits.

3. Configure the PXIe-4135 for Measuring

Once you configure channels and they are in the Running state, the PXIe-4135 cantake measurements.
...
5. Initiate the PXIe-4135 for Sourcing and MeasuringInitiate the channels of the PXIe-4135 to apply a configuration and start generating.

6. Acquire MeasurementsThe applied channel configuration determines how the PXIe-4135 acquiresmeasurements.

7. Cease GenerationNI-DCPower includes different options for stopping generation on PXIe-4135channels and returning the channels to a known state.

8. Close the SessionUse the NI-DCPower driver to close a session with the PXIe-4135.
...
For any application you write, you must open a session to establish communicationwith the PXIe-4135 or specified channel(s) by initializing.
...
# Configure the PXIe-4135 for Sourcing

Use the NI-DCPower driver with the PXIe-4135 to control the output the instrumentgenerates. Depending on the output function and source mode, you can configure theappropriate output levels and limits.
...
# Configure the PXIe-4135 for Measuring

Once you configure channels and they are in the Running state, the PXIe-4135 can takemeasurements.
...
Refer to the PXIe-4135 Specifications for the trigger delay and jitter of yourinstrument.
...
# Initiate the PXIe-4135 for Sourcing and Measuring

Initiate the channels of the PXIe-4135 to apply a configuration and start generating.
...
The applied channel configuration determines how the PXIe-4135 acquiresmeasurements.
...
The PXIe-4135 automatically acquires measurements when you configure thefollowing VIs or functions:
...
NI-DCPower includes different options for stopping generation on PXIe-4135 channelsand returning the channels to a known state.
...
The output of a channel is enabled by default when the channel enters the Runningstate. However, you can programmatically enable and disable the output channel(s) ofthe PXIe-4135.

When you disable the output of the PXIe-4135, the instrument is configured to output aDC voltage at 0 V with current limits at $\pm 2 \%$ of the presently configured current limitrange in, unless otherwise noted, a low-impedance state.
...
Use the NI-DCPower driver to close a session with the PXIe-4135.
...
The PXIe-4135 can perform common SMU operations to pulse voltage and current. Inorder to perform these operations, use the NI-DCPower driver to configure softwaresettings and execute operations.

Refer to the following table for an overview of common SMU source and measureoperations as well as the software setting combinations that enable the PXIe-4135 toperform each operation.


Table 10. Software Settings for PXIe-4135 Pulse Operations


<table><tr><td rowspan="2">PXle-4135 Operation</td><td colspan="2">Software Settings</td></tr><tr><td>Output Function</td><td>Source Mode</td></tr><tr><td>Pulse voltage</td><td>Pulse Voltage</td><td rowspan="2">Single Point or Sequence</td></tr><tr><td>Pulse current</td><td>Pulse Current</td></tr></table>
...
2. Configure the PXIe-4135 for Pulsing
...
5. Initiate the PXIe-4135 for Sourcing and Measuring
...
NI-DCPower includes different options for stopping generation on PXIe-4135channels and returning the channels to a known state.
...
# Configure the PXIe-4135 for Pulsing
...
<table><tr><td>Measure When</td><td>Details</td></tr><tr><td>On Demand</td><td>Acquire measurements on demand using the niDCPower Measure VI and the niDCPower_Measure function to measure either the voltage or the current on a single channel. Or use the niDCPower Measure Multiple VI and the niDCPower_MeasureMultiple function to measure both the voltage and the current on multiple channels. When you call these VIs and functions, the PXle-4135 takes a measurement and returns it.</td></tr><tr><td>Automatically after Source Complete</td><td>The PXle-4135 acquires a measurement after every source operation and stores it in a buffer on the device. You can use the niDCPower Fetch Multiple VI and the niDCPower_FetchMultiple function to retrieve measurements from the buffer.</td></tr><tr><td>On Measure Trigger</td><td>The PXle-4135 acquires a measurement when it receives a Measure trigger and stores it in a buffer on the device. You can use the niDCPower Fetch Multiple VI and the niDCPower_FetchMultiple function to retrieve measurements from the buffer.</td></tr></table>
...
When you disable the output of the PXIe-4135, the instrument is configured to output aDC voltage at 0 V with current limits at $\pm 2 \%$ of the presently configured current limit
...
Refer to the following sections for information about PXIe-4135 features and guidelinesfor operating the PXIe-4135.
...
Because of the required power dissipation, sourcing and sinking capabilities for achannel are not always identical. Refer to the PXIe-4135 Specifications for more
...
Cable inductance has an effect on the output current slew rate. You can decreasecabling inductance and increase the speed of the PXIe-4135.
...
You can program the channel of the PXIe-4135 to vary the output resistance.
...
niDCPower_Configure_Output_Resistance function to set the outputresistance. Refer to the PXIe-4135 Specifications for more information about thevalues to which you can set the output resistance.
...
channel and disconnects the channel circuitry from the output connector pins. Byinternally disconnecting the output, it protects both the PXIe-4135 and the deviceunder test (DUT).
...
Do not apply voltages at the output that exceed the ratings of the PXIe-4135. Refer tothe PXIe-4135 Specifications for information about voltage ratings.
...
The following table lists the PXIe-4135 transient response settings available in NI-DCPower.
...
The following table lists the compensation parameter settings for the PXIe-4135. Youcan independently set the parameters for constant voltage mode and constant current
...
Use the highest resolution (smallest) range possible for a particular application to getmaximum output and measurement accuracy. Refer to the PXIe-4135
...
The PXIe-4135 supports SourceAdapt custom transient response, which allows you toset compensation parameters to fine-tune the transient response of a channel for yourspecific application.
...
Each channel of the PXIe-4135 has two synchronized ADCs that measure voltage andcurrent. You can use NI-DCPower to measure power flowing to or from the PXIe-4135.

You can use the following VIs and functions to measure both current and voltage forboth channels of the PXIe-4135.
...
measurement is positive, the PXIe-4135 is sourcing power. If the power measurementis negative, the PXIe-4135 is sinking power.
...
You can integrate the PXIe-4135 into a safety interlock system.
...
The PXIe-4135 PXI source measure units include this safety interlock functionality.
...
Table 17. NI SMU Safety Interlock Voltage Thresholds and Behavior
...
The PXIe-4135 is capable of generating hazardous voltages and working withinhazardous voltage systems. It is the responsibility of the system designer, integrator,installer, maintenance personnel, and service personnel to ensure the system is safeduring use.

• Ensure operators cannot access the PXIe-4135, cables, the device under test (DUT),or any other instruments in the system while hazardous voltages are present.Operator access points can include, but are not limited to, guards, gates, slidingdoors, hinge doors, lids, covers, and light curtains.

• If using a test fixture enclosure, ensure that it is properly connected to safetyground.

• Ensure that the PXIe-4135 is properly secured to the chassis using the two frontpanel mounting screws.
...
To ensure a system containing the PXIe-4135 is safe for operators, components, orconductors, take the following safety precautions:
...
• Before touching any of the connections to the high terminal or high sense on thePXIe-4135, discharge all components connected to the measurement path. Verifywith a DMM before interaction with connections.
...
Table 18. Safety Interlock Cable for PXIe-4135
...
Complete the following steps to wire the safety interlock subsystem using the SafetyInterlock Cables for PXIe-4135.
...
Refer to the PXIe-4135 Specifications for the lead drop, the maximum voltagedrop allowed on an output lead before a sense lead error is detected.
...
Refer to the PXIe-4135 Specifications for measurement resolution and outputresolution information.
...
Refer to the PXIe-4135 Specifications for the following information for yourinstrument:
...
Refer to the PXIe-4135 Calibration Procedure for the external calibrationprocedure for your instrument.
...
• After first installing the PXIe-4135 in a chassis

• When the PXIe-4135 is in an environment where the ambient temperature changes.Refer to the PXIe-4135 Specifications to find the allowable ambienttemperature for your instrument.

• When the PXIe-4135 temperature has drifted outside of the specified $\mathsf { T } _ { \mathsf { C a l } }$ since thelast self-calibration. Refer to the PXIe-4135 Specifications to find the allowabledifference from $\mathsf { T } _ { \mathsf { C a l } }$ for this instrument.
...
• Due to high-impedance circuits used in the hardware, care should be taken toavoid contamination during handling or operation. Avoid use or storage of thehardware in an environment that allows dust to settle on the hardware. Avoiddirect contact with the inner surfaces of triax connections. Triax covers should beused whenever triax connections are not in use.